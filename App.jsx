...
【重要: menuListとdaysの整合性ルール】
- menuListには「主菜名」と「副菜名」のみ記載する（主食は含めない）
- menuListの名前は、daysの各食事のmain.nameまたはside.nameと完全一致させること
- 例: menuListに「鶏そぼろ」を入れたら、daysのmain.nameにも「鶏そぼろ」と書く
- 「鶏そぼろ丼」のような主食+主菜の複合名は使わず、主菜は「鶏そぼろ」、主食は「ごはん」と分けること

以下のJSON形式のみで返してください（他のテキスト不要）:
\`\`\`json
{
  "cookingTips": "まとめ調理のコツを1〜2文で",
  "menuList": ["主菜名1", "主菜名2", "副菜名1", "副菜名2"],
  "prepTimeline": [
  ...
  "days": [
    {
      ...
      "meals": {
        "朝食": {
          ...
          "main":   { "name": "主菜名（menuListと一致）", "desc": "20文字以内", "baby": "子どもアレンジ15字以内" },
          "side":   { "name": "副菜名（menuListと一致）", "desc": "20文字以内" }
        },
        "昼食": {
          "staple": { "name": "主食名", "desc": "20文字以内" },
          "main":   { "name": "主菜名（menuListと一致）", "desc": "20文字以内", "baby": "子どもアレンジ15字以内" },
